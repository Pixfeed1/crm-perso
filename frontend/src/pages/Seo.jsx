...
const Seo = () => {
  ...
  const [tab, setTab] = useState('jus'); // 'jus' | 'affamees'
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [ov, gr, pg, af] = await Promise.all([
        seoAPI.getOverview(siteId),
        seoAPI.getGraph(siteId),
        seoAPI.getPages(siteId, { sort, limit: 500 }),
        seoAPI.getAffamees(siteId)
      ]);
      setOverview(ov); setGraph(gr || { nodes: [], edges: [] }); setPages(pg || []); setAffamees(af || []);
    } catch (e) {
        ...
        {sites.length === 0 && !loading ? (
          ...
          <>
            ...
            <div className="flex gap-2 mb-4">
              {[{ k: 'jus', l: 'Jus interne' }, { k: 'affamees', l: `Pages affamées${affamees.length ? ` (${affamees.length})` : ''}` }].map((t) => (
                  ...
                  <motion.div key={p.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {p.suggestions && p.suggestions.length > 0 && (
                      <div className="mt-3 border-t border-border/60 pt-2">
                        <div className="text-xs text-text-muted mb-1 flex items-center gap-1"><FiLink size={12} /> Liens internes suggérés (depuis ces pages à fort jus) :</div>
                        <ul className="space-y-1">
                          {p.suggestions.map((s) => (
                            <li key={s.url} className="text-sm text-text-secondary flex items-center gap-1.5">
