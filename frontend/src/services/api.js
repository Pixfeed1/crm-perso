...
export const seoAPI = {
  ...
  // type = web (defaut) | image | discover : un rang d'image n'est pas un rang de lien bleu.
  getPositionsSummary: (siteId, days = 28, type = 'web') => apiRequest(`/seo/positions/summary?site_id=${siteId}&days=${days}&type=${type}`),
  getPositionsKeywords: (siteId, params = {}) => {
    const qs = new URLSearchParams({ site_id: siteId, ...params }).toString();
    return apiRequest(`/seo/positions/keywords?${qs}`);
  },
  getPositionKeywordSeries: (siteId, keyword, days = 28, type = 'web') =>
    apiRequest(`/seo/positions/keyword?site_id=${siteId}&keyword=${encodeURIComponent(keyword)}&days=${days}&type=${type}`),
  getPositionsPages: (siteId, days = 28, type = 'web') => apiRequest(`/seo/positions/pages?site_id=${siteId}&days=${days}&type=${type}`),
  getPositionsPage: (siteId, url, days = 28, type = 'web') =>
    apiRequest(`/seo/positions/page?site_id=${siteId}&url=${encodeURIComponent(url)}&days=${days}&type=${type}`),
