...
const sendReminderEmail = async (db, invoice, reminderLevel) => {
  try {
    ...
    console.error(`❌ Erreur lors de l'envoi de la relance pour facture ${invoice.invoice_number}:`, error);
...
const sendBatchReminders = async (db, invoices) => {
  ...
  console.log(`\n📧 Envoi batch terminé : ${results.sent} réussis, ${results.failed} échecs`);
